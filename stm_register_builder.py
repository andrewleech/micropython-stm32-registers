...
def __register_file_generator(stm_cpu, requirements, list_instead=False, out_file="_stm_registers.py"):
    """
    ...
    if len(includes) > 1:
        ...
        raise ValueError(f"No matching header found, known targets are: {', '.join(sorted(valid_cpus))}")
    else:
        filename = includes[0]
    ...
    if len(requirements) == 1 and Path(requirements[0]).exists():
        ...
        for field in requirements:
            if "*" in field:
                ...
                __desired__.append(
                    partial(lambda v, f: f == v, f=field)
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument("requirements", nargs="*", help="list of registers to include (or file to parse for registers).")
    parser.add_argument("--list", action="store_true", help="List all avalable registers for target chip")
    parser.add_argument("--out", default="_stm_registers.py", help="Set the path/name for the generated file")

    args = parser.parse_args()

    __register_file_generator(args.target, args.requirements, list_instead=args.list, out_file=args.out)
    sys.exit()
